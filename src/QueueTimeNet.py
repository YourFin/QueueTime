...
def QueueTime_loss(y_true, y_pred): # should be a CELL_ROW * CELL_COL * 5 tensor
	...
	x_loss = K.square(y_true[...,1] - y_pred[...,1]).eval()
	print("[INFO] get here", indicator)
	y_loss = K.square(y_true[...,2] - y_pred[...,2]).eval()
	print("[INFO] get here2", indicator)
	xy_loss += coord * np.sum(indicator*(y_loss+x_loss), None)
	...
	pr_loss_neg = 0
	pr_loss_pos = 0
	pr_loss_pos += np.sum(indicator * K.square(indicator - y_pred[...,0]).eval())
	pr_loss_neg += noobj*np.sum((1-indicator) * K.square(indicator - y_pred[...,0]).eval())
